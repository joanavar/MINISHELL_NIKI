...
/*   Updated: 2025/01/31 11:39:08 by nikitadorof      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	create_redir_token(char *str, int i, char *token)
{
	if (str[i] == '<' && str[i + 1] == '<')
	{
		token[0] = '<';
...
static int	is_redireccion(char *str, int i, t_token **stack)
{
	...
	if (!token)
		return (0);
	if (create_redir_token(str, i, token))
		token[2] = '\0';
	if (!get_token(token, stack))
	{
		free(token);
...
static int	handle_special_char(char *str, int *i, t_token **stack)
{
	if (str[*i] == ' ')
		return (is_caracter_token(str[*i], stack));
	if (str[*i] == '|')
		return (is_caracter_token(str[*i], stack));
	if (str[*i] == '>' || str[*i] == '<')
	{
...
static int	lectur_line(char *str, t_token **stack, int i)
{
	while (str[i])
	{
		...
		{
			if (!handle_special_char(str, &i, stack))
				return (0);
		}
		else
		{
